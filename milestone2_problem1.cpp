...
void encode_file(char array[], int size)
{
	char ch;
	int char_position;
	...
	for (int i = 0; i < size; i++) {
		ch = ("%s", array[i]);
		char_position = search_name_in_list(ASCII_SYMBOLS, array[i], 128);
		outfile << ASCII_CODE[char_position];
	}
	outfile.close();
}
