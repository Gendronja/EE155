...
{
	int code_position = search_name_in_list(ASCII_CODES, code, 128);
...
// Copy-pasted from encode_file_huffman(), text files changed to encode in ascii
void encode_file_ascii()
{
	int symbol_position;
	char c;
	string ch;
	...
	ofstream encoded("macbeth.ascii");
...
void decode_file()
{
	...
	for (int i = 0; i < 128; i++) {
		create_node(root, ASCII_CODES[i]);
	}
	TREE_NODE* element = root;

	ifstream to_decode("macbeth.ascii");
	ofstream decoded("macbeth.ascii.decoded");
